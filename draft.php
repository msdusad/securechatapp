...
<div class="w3-col m8 w3-" style="padding-left:1px;padding-right:1px;padding-bottom:20px">
    ...
    <h4 class="btn btn-default"><i class="fa fa-dropbox"></i> Draft</h4>
    </div>
        </div>
        <div class="w3-col m6">
...
$sql=mysql_query("select * from messages where sender_id='$user' && reciver_type='Draft' order by send_time desc;");
if($sql){
    ...
    <div class='w3-row'>
      <div class='w3-col w3-right w3-' style='width:75px;padding-top:7px'>";
        if($attach!=''){
      echo "<a  title='Download File' href='downloadfile.php?tokid=$tokenid&t=$time&sdid=$from&recid=$reciverid'><i class='fa fa-file'></i></a>";
        }
     echo " </div>
        <div class='w3-rest w3-'  style='padding-top:7px;padding-left:5px'><p><a title='$subject' href='view_message.php?type=draft&tokid=$tokenid&t=$time&sdid=$from&recid=$reciverid' style='text-decoration:none;color:black;'><b>Subject :</b> $showsubject</a></p></p></div>
    </div>
  </div>
           <div class='w3-quarter w3-' style='padding-top:7px;padding-left:5px'><p></p></div>
  <div class='w3-quarter w3-'  style='padding-top:7px;padding-right:5px'><p>$messagetime <a href='code/delete-message.php?type=draft&time=$time&tok=$tokenid'><i class='fa fa-trash pull-right'></i></a></p></div>
</div><br>";
